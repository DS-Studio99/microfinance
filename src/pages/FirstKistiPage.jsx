...
// ── Single Member Row ───────────────────────────────────
const FirstKistiMemberRow = ({ member }) => {
  return (
    <div style={{
      display: 'grid',
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <span style={{ background: '#e0e7ff', color: '#4338ca', padding: '4px 10px', borderRadius: 10, fontSize: 11, fontWeight: 700, fontFamily: "'Hind Siliguri', sans-serif", display: 'inline-block' }}>
          ১ম কিস্তি: {formatDate(member.firstKistiDate)}
        </span>
      </div>
    </div>
  )
...
// ── VO Accordion Card ───────────────────────────────────
const VOFirstKistiCard = ({ vo, members, idx, navigateToVO }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', borderBottom: expanded ? '1px solid #f1f5f9' : 'none' }}>
        <button
          ...
          style={{
            ...
            padding: '0.65rem', border: 'none', background: '#fafafa', color: '#4338ca',
      ...
      {expanded && (
        <div>
          {members.map(m => <FirstKistiMemberRow key={m.id} member={m} />)}
        </div>
      )}
    </div>
  )
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      ...
      ;(data || []).forEach(m => {
        ...
        const firstKistiDate = addMonthsToDate(d, 1)
        if (firstKistiDate.startsWith(currentYM)) {
          // Check if this month's installment has been paid:
          // loan_payment_date being set to this month or earlier means it's still pending/due
          // If loan_payment_date is null = installment was marked paid (paid and cleared for this cycle)
          // We want to show UNPAID: those who have loan_payment_date != null (still due)
          // OR those where last_paid_date does NOT start with currentYM (haven't paid this month)
          const paidThisMonth = m.last_paid_date && m.last_paid_date.startsWith(currentYM)
          if (!paidThisMonth) {
            list.push({ ...m, loan_disbursement_date: disbDateStr, firstKistiDate })
          }
        }
      })
      ...
      console.error('First kisti fetch error:', err)
    } finally {
      setLoading(false)
    }
  }, [])
    ...
    <div className="page-enter">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 13, color: '#64748b', fontFamily: "'Hind Siliguri', sans-serif", marginTop: 2 }}>
            ঋণ বিতরণের ১ মাসের ১ম কিস্তির ভিও-ভিত্তিক রিপোর্ট
          </p>
        </div>
        <button
      ...
      {!loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: '1.25rem' }}>
          {[
            { icon: MdPeople, label: '১ম কিস্তির সদস্য', value: filteredMembers.length, g1: '#4f46e5', g2: '#3730a3' },
            { icon: MdGroups, label: 'ভিও সংখ্যা', value: byVO.length, g1: '#059669', g2: '#047857' },
            { icon: MdCalendarToday, label: 'ধাপ', value: '১ম মাস', g1: '#0891b2', g2: '#0e7490' },
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: 20, border: '1px solid #e2e8f0' }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: '#64748b', fontFamily: "'Hind Siliguri', sans-serif" }}>
            ১ম কিস্তির কোনো সদস্য পাওয়া যায়নি
          </p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {byVO.map(({ voNum, members: mems, vo }, idx) => (
            <VOFirstKistiCard key={voNum} vo={vo} members={mems} idx={idx} navigateToVO={(num) => navigate(`/vo/${num}`)} />
          ))}
        </div>
      )}
    </div>
