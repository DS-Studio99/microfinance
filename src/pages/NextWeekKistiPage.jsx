...
import {
  ...
  MdExpandMore, MdExpandLess, MdRefresh, MdChevronRight, MdEventAvailable, MdCheckCircle, MdVerified
} from 'react-icons/md'
...
const KistiMemberRow = ({ member }) => {
  ...
  const isTargetExpected = member.expected_payment_date === targetDate
  const isTargetRegular = member.loan_payment_date === targetDate
  ...
  return (
    <div style={{
      ...
      background: isTargetExpected ? '#f0fdf4' : (days >= 30 ? '#fff9f9' : '#fff'),
      alignItems: 'center',
    }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {isTargetRegular && (
            <span style={{ background: '#ccfbf1', color: '#115e59', fontSize: 10, fontWeight: 700, borderRadius: 6, padding: '1px 6px', fontFamily: "'Hind Siliguri', sans-serif" }}>
              নির্ধারিত কিস্টি
            </span>
          )}
        </div>
      ...
      {/* Days overdue badge */}
      {days && sev && !isTargetRegular && !isTargetExpected && (
        <div style={{
      ...
      {isTargetExpected && (
         <div style={{ background: '#22c55e', color: '#fff', borderRadius: 10, padding: '5px 10px', textAlign: 'center', flexShrink: 0, minWidth: 64 }}>
           ...
           <div style={{ fontSize: 9, fontWeight: 700, marginTop: 1, fontFamily: "'Hind Siliguri', sans-serif" }}>৭ দিন পর</div>
         </div>
      )}
      {isTargetRegular && (
         <div style={{ background: '#14b8a6', color: '#fff', borderRadius: 10, padding: '5px 10px', textAlign: 'center', flexShrink: 0, minWidth: 64 }}>
           <MdEventAvailable style={{ fontSize: 20 }} />
           <div style={{ fontSize: 9, fontWeight: 700, marginTop: 1, fontFamily: "'Hind Siliguri', sans-serif" }}>৭ দিন পর</div>
         </div>
      )}
    </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
          <div style={{ background: 'rgba(255,255,255,0.25)', borderRadius: 20, padding: '3px 10px', display: 'flex', alignItems: 'center', gap: 5 }}>
            <MdEventAvailable style={{ color: '#fff', fontSize: 13 }} />
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      ...
      const filtered = (data || []).filter(m => 
        (m.loan_payment_date && m.loan_payment_date === targetDate) || 
        (m.expected_payment_date && m.expected_payment_date === targetDate)
      )
      ...
      <div className="page-enter">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#0f172a', fontFamily: "'Hind Siliguri', sans-serif" }}>
              আগামী ৭ দিনের কিস্তি
            </h1>
            <p style={{ fontSize: 13, color: '#64748b', fontFamily: "'Hind Siliguri', sans-serif", marginTop: 2 }}>
              আগামী ১ সপ্তাহ পর যাদের কিস্তির তারিখ তাদের তালিকা (অ্যাডভান্স মনে করিয়ে দেয়ার জন্য)
            </p>
          </div>
          <button
        ...
        {!loading && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: '1.25rem' }}>
            {[
              {
                icon: MdPeople, label: 'মোট কিস্তি (৭ দিন পর)',
        ...
        {!loading && kistiMembers.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '5rem 1rem', textAlign: 'center' }}>
            <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'linear-gradient(135deg,#ccfbf1,#99f6e4)', border: '2px solid #5eead4', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
              <MdEventAvailable style={{ fontSize: 38, color: '#0f766e' }} />
            </div>
            <h3 style={{ fontSize: 20, fontWeight: 800, color: '#134e4a', fontFamily: "'Hind Siliguri', sans-serif", marginBottom: 8 }}>
              ৭ দিন পর কোনো কিস্তি নেই! 🎉
            </h3>
            <p style={{ fontSize: 14, color: '#64748b', fontFamily: "'Hind Siliguri', sans-serif" }}>
              আগামী সপ্তাহে কোনো সদস্যের কিস্তি সংগ্রহের তারিখ নেই।
            </p>
          </div>
        )}
        ...
        {!loading && byVO.length > 0 && (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              <MdGroups style={{ color: '#0f766e', fontSize: 18 }} />
              <p style={{ fontSize: 12, fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: 0.7, fontFamily: "'Hind Siliguri', sans-serif" }}>
                ভিও-ভিত্তিক কিস্তির তালিকা (আগামী ৭ দিন)
              </p>
...
export default NextWeekKistiPage
